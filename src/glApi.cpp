...
{
  const std::string & file = fileContent(filename);
  const char * code = file.c_str();
  GLint size = file.size();
  m_location = glCreateShader(type);
  glShaderSource(m_location, 1, &code, &size);
...
template <> void Texture::setData<GLubyte>(const Image<GLubyte> & image, bool mipmaps) const
{
  ...
  case 4 : format = GL_RGBA;
    break;
  default :
    throw "no format compatible";
  }
  if(m_target == GL_TEXTURE_2D){
    glTexImage2D(m_target,0,format, image.width, image.height,0,format, GL_UNSIGNED_BYTE, image.data);
  }
  if(m_target==GL_TEXTURE_3D){
    glTexImage3D(m_target, 0,format, image.width, image.height, image.depth, 0,format, GL_UNSIGNED_BYTE, image.data);
  }
  if(mipmaps){
...
void Sampler::enableAnisotropicFiltering() const
{
  ...
  setParameter(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT,maxAniso);
}
